...
function showMap(resp,enumx){
    ...
    if (resp.length>0) {
     // spinner.hide()
      ...
      resp.map(function(item){
        ...
        if (item.Customer_Coord.match("/(.*)")[1]) {
           console.log('d',item.Customer_Coord.match("/(.*)")[1])
           var card=enumx.some(function(en){
            console.log("ddzs",en.enum_id);
            return (en.enum_id == item.feederID);
           })
           console.log("mapl",card)
           if (card) {
            locations.push([item.feeder_name,item.Customer_Coord.match("(.*)/")[1],item.Customer_Coord.match("/(.*)")[1],'http://localhost/phed_outage/assets/img/map/red_MarkerM.png'])
           } else {
            locations.push([item.feeder_name,item.Customer_Coord.match("(.*)/")[1],item.Customer_Coord.match("/(.*)")[1],'http://localhost/phed_outage/assets/img/map/darkgreen_MarkerA.png'])
           }
  ...
  $(document).ready(function(){
    var url=BASE_URL+"mis/get_all_customers";
    $.ajax({
        url:url,
        type:'POST',
        data:"",
        success:function(response){
            ...
            if (data.status) {
              ...
              showMap(data.customers,data.faults)
