...
function showMap(assetType,assetid){
  //alert(assetid)
   ...
   /// spinner.show();
      ...
      //alert(assetid)
      $('#map').html('<center><div class="alert alert-info my-5"><span class="fa fa-spinner fa-spin"></span> Loading customers coordinates...</div></center>')
$.post("http://10.10.25.31:8084/nomsapiwslim/v1/getCustomersByAssets",
  ...
  function(resp, status){
    ...
    if (resp.length>0) {
     // spinner.hide()
      ...
      resp.map(function(item){
        locations.push([item.feeder_name,item.Customer_Coord.match("(.*)/")[1],item.Customer_Coord.match("/(.*)")[1]])
      })
    ...
    $("#map").css({"width": "100%","height":"600px"});
              ...
              //spinner.hide();
              alert("Oops! This feeder is not mapped");
